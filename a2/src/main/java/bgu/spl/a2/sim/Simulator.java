...
public class Simulator {
    ...
    public static void start() {
        ...
        // Phase 1
        runPhaseActions(jsonObject.get("Phase1").getAsJsonArray());
        // Phase 2
        runPhaseActions(jsonObject.get("Phase2").getAsJsonArray());
        // Phase 3
        runPhaseActions(jsonObject.get("Phase3").getAsJsonArray());
    }
    ...
    private static void runPhaseActions(JsonArray array) {
        ...
        for (JsonElement element : array) {
            ...
            if (jsonObject.get("Action").getAsString().equals("Participate In Course")) {
                ...
                action = new RegisterWithPreferences(toList(jsonObject.get("Preferences").getAsJsonArray()), toList(jsonObject.get("Grades").getAsJsonArray()));
                actorThreadPool.submit(action, jsonObject.get("Student").getAsString(), new StudentPrivateState());
            } else if (jsonObject.get("Action").getAsString().equals("Check dministrative Obligations")) {
